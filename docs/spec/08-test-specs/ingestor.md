...
| ING-U-011 | `CsvSource` mapping where every non-`None` column is present in the header (`system_column` unset) | Construction succeeds; the mapping-validation loop exits normally and skips the `None` column without raising (loop normal-exit + `None` short-circuit branches) |
| ING-U-012 | `JsonlSource` given a nonexistent file URI | `SourceConfigError` at construction (`OSError` → exit 2), same bucket as an unreadable CSV source URI |
