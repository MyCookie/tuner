...
| CLN-U-011 | `clean_command` invoked via `CliRunner` with no `--run-id` | Usage error naming the missing option — the click wiring itself is exercised at least once (docs/08-test-specs/README.md's CliRunner convention), not just `clean_record`/`scrub` directly |
