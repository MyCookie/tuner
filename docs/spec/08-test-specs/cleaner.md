...
| CLN-U-011 | `clean_command` invoked via `CliRunner` with no `--run-id` | Usage error naming the missing option — the click wiring itself is exercised at least once (docs/spec/08-test-specs/README.md's CliRunner convention), not just `clean_record`/`scrub` directly |
...
² **Design decision (T07 review rounds 1-3):** `_is_well_structured` ([rules.py](../../../src/tuner/cleaner/rules.py)) re-derives the Multimodal Contract's per-part shape rules (valid `type` enum, `value` is a string, no keys beyond `type`/`value`) rather than delegating to `ContentPart`'s own pydantic validator, because that validator's non-empty-after-trim check would fire on the *pre-scrub* value — collapsing the deliberate `unmappable` (empty at the source) vs. `empty_turn` (blank only after scrub) distinction CLN-U-007 requires. The trade-off is that `_is_well_structured` must independently track every shape rule the contract enforces, checked defensively since a JSONL `raw.conversation` is untrusted, attacker- or typo-controlled input — this took three review rounds to actually get complete: round 1 found it missing the content-part type enum, value type, and extra-key checks; round 2 found the same extra-key gap one level up, on the turn dict itself; round 3 found that the two enum membership tests (`role in _VALID_ROLES`, `type in _VALID_CONTENT_TYPES`) raised an uncaught `TypeError` instead of returning `False` when the value was an unhashable JSON type (a list or object) rather than a plain string, crashing the whole run instead of dropping the one bad record. Round 3 also fuzzed 200,000 mutated conversations through `clean_record` and validated every survivor against `SilverGoldRecord` to confirm no fourth gap remained.
