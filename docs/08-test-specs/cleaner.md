...
| CLN-U-008 | `bad_structure`: no assistant turn, last turn user, double system, system not first, a single turn, and every other way a contract-shaped `raw.conversation` can be malformed while still being a JSON value — non-dict turn, non-array content, non-dict content part, missing/invalid `type`, non-string `value`, an extra key, an invalid role (parametrized)² | Drop `bad_structure` — cleaner drops, not crashes, since Bronze `raw` is untrusted |
