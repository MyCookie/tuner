...
- **`TRN-I-004` never checked any URI the manifest actually contains.** It read the manifest and asserted `status`/`model_version`/`adapter_name`/`base_model`/`method`, then independently *reconstructed* the gold-manifest, index-map, and weights keys from `tokenized_run_id`/`model_version` rather than reading `manifest.gold_manifest_uri`/`manifest.index_map_uri`/`manifest.weights_uri` and resolving those values. Proven live: mutating `weights_uri` in `trainer/cli.py` to `"s3://wrong-bucket/does-not-exist/"` left the full 17-test suite green. Fixed with a small `_parse_s3_uri` helper; the test now parses each of the three manifest URI fields and resolves *that* bucket/key against real storage, so a wrong value in any of them now fails the test (re-confirmed with the same mutation after the fix).
- **`bf16=torch.cuda.is_available()` checked only device presence, not the device's own bf16 support.** Some CUDA devices lack bf16 tensor cores and would hit the exact `Your setup doesn't support bf16/gpu` failure the round-1 fix was meant to eliminate. Fixed to `torch.cuda.is_available() and torch.cuda.is_bf16_supported()`.
- **Idempotent re-run only deleted the current method's own output subdir.** If `train.method` flips between two runs sharing a `run_id` (e.g. `full` then `qlora`), the first method's output (`model/` or `adapter/`) would never be deleted, orphaning it under the run prefix forever — a violation of CLAUDE.md hard rule 4's "delete own output prefix ... rewrite" for the *whole* prefix, not just the slice the current method happens to use. Fixed to delete both `model/` and `adapter/` before uploading.
- **A failure between opening the temp work dir and starting the MLflow run (token download, SafeTensors load, reaching the tracking server) escaped as a raw traceback**, not the `train: ...` message every other failure mode produces — exit code 1 held either way, but the message didn't. Fixed by widening the surrounding `try` to start before that block instead of at `mlflow.start_run`.
- **`TRN-I-001`'s finite-loss check (`x == x`) rejects `NaN` but not `inf`.** A runaway/overflowed loss would pass silently. Fixed to `math.isfinite(...)`.
