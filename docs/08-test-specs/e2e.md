...
Spec under test: the whole MVP ([00 §2 success criteria](../00-product-scope.md), [06 §5](../06-testing.md)). File: `tests/e2e/test_steel_thread.py`, config `configs/pipeline.e2e.yaml` (tiny-test adapter, mock judge as compose sidecar, real MinIO + MLflow server). One pipeline execution, then one assertion function per case below (single `tuner run`, many asserts — budget ≤10 min GPU / nightly CPU).

| ID | Assertion over the completed run | Expected |
| :--- | :--- | :--- |
| E2E-E-001 | Exit status | `tuner run` returned 0 |
...
| E2E-E-006 | Registry | Exactly one manifest, `status: candidate`, all URIs resolve, visible in `tuner registry list` |
...
| E2E-E-009 | Idempotent stage re-run | Re-running `tuner tokenize` + `tuner train` for the same run ID leaves one coherent set of artifacts and an unchanged-or-replaced single registry manifest |
| E2E-E-010 | IAM spot-check | Cleaner creds denied writing `tuner-gold`; trainer creds denied reading `tuner-bronze` |
