...
- **Precedence:** the mock checks markers in a fixed order — `[[fail=...]]` first, then `[[garbage]]`, then `[[score=N]]`, else the score-8 default — so a record can't accidentally suppress an injected failure by also matching another marker.
- **Marker scope:** the mock scans the *entire* rendered request (every message's content) for a marker, not just the record's own text. When T08 writes `RUBRIC_V1` (`src/tuner/judge/prompts.py`), its instructional text must not itself contain anything shaped like `[[...]]` — e.g. don't use that bracket style in a worked example of the desired output — or every call would match it as an unintended marker.
- **Scope of this test double, T05 vs. T08:** the mock is intentionally stateless — it has no memory of prior calls. `JDG-I-022` (retried-call counting), `JDG-I-024` (fail-once-then-succeed), and `JDG-I-025` (peak in-flight count) need call-count/attempt-number state; T05's marker vocabulary above does not cover them. T08 extends `tests/mock_judge/app.py` with that state when it builds those cases, scoped so it doesn't change the T05 marker behavior already covered by `JDG-U-001..004`.
