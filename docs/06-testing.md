...
- **Marker scope:** the mock scans the *entire* rendered request (every message's content) for a marker, not just the record's own text. `RUBRIC_V1` (`src/tuner/judge/prompts.py`)'s instructional text does not contain anything shaped like `[[...]]` — e.g. no worked example uses that bracket style — or every call would match it as an unintended marker.
