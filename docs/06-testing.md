...
- Adapter `tiny-test` — a permanently registered adapter over **`HuggingFaceTB/SmolLM2-135M-Instruct`** (135M params, chat-templated, ungated; `hf_revision` pinned to `12fd25f77366fa6b3b4b768ec3050bf629380bac`, confirmed against the live repo via the HF API at T09 implementation time, 2026-08-23), `supports_full_ft: True`. Test infrastructure, not a product model — it exists so CPU-runnable tests exercise real tokenizer/trainer code paths. This is also the model whose cache CI pre-seeds for offline mode (INF-I-012).
