...
Authoritative architecture reference for the Enterprise Fine-Tuning Pipeline (EFTP). This document also fixes the **glossary** — the canonical names for buckets, environment variables, config keys, CLI commands, and identifiers. Every other document uses these names verbatim; if a name here conflicts with a name elsewhere, this document wins.
...
EFTP is a decoupled pipeline of stateless stages. Each stage:

1. Is invoked as a CLI (`eftp <stage> --run-id <RUN_ID> --config <path>`).
...
| :--- | :--- | :--- | :--- |
| Ingestor | external sources | `eftp-bronze` | [ingestor.md](03-components/ingestor.md) |
| Cleaner | `eftp-bronze` | `eftp-silver` | [cleaner.md](03-components/cleaner.md) |
| Judge | `eftp-silver` | `eftp-gold` | [judge.md](03-components/judge.md) |
| Tokenizer | `eftp-gold` | `eftp-artifacts` | [tokenizer.md](03-components/tokenizer.md) |
| Trainer | `eftp-artifacts` | `eftp-artifacts`, `eftp-registry` | [trainer.md](03-components/trainer.md) |
| Smoke-test | `eftp-artifacts` | `eftp-artifacts` | [smoke-test.md](03-components/smoke-test.md) |
| Registry ops | `eftp-registry` | `eftp-registry` | [registry.md](03-components/registry.md) |
| Inference Engine | `eftp-registry` | — (serves traffic) | [inference.md](03-components/inference.md) |
...
- **MVP — local driver:** `eftp run --config configs/pipeline.yaml` executes the stages in order as subprocesses, aborting on the first non-zero exit code. It generates the run ID, passes it to every stage, and prints the final artifact locations. The driver is intentionally dumb: no retries, no partial resume in MVP (re-running a stage manually with the same run ID is the recovery path — stages are idempotent, see §5.3).
...
├── pyproject.toml             # single package: eftp (managed with uv)
...
│   ├── trainer/
│   ├── smoke/
│   ├── registry_ops/          # `eftp registry ...` subcommands
│   └── cli.py                 # `eftp` entrypoint dispatching to stage CLIs + `eftp run`
└── tests/
    ├── unit/
...
| Bucket | Tier | Content |
| :--- | :--- | :--- |
| `eftp-bronze` | Bronze | raw records in metadata envelope |
| `eftp-silver` | Silver | cleaned Multimodal Contract records |
| `eftp-gold` | Gold | judged/filtered records |
| `eftp-artifacts` | Artifact | tensors, adapters, transcripts |
| `eftp-registry` | Registry | model version manifests |
| `eftp-mlflow` | (infra) | MLflow artifact store — server-internal, no stage touches it directly |
| `eftp-assets` | (Phase 4) | binary media referenced by multimodal records |
...
- **Run ID**: `run-{YYYYMMDD}-{HHMMSS}-{6 lowercase hex}` (UTC), e.g. `run-20260720-142201-a3f9c2`. Generated once by the orchestrator, passed to every stage as `--run-id`, and threading through every manifest, MLflow run tag (`eftp.run_id`), and registry entry.
...
| :--- | :--- | :--- |
| `EFTP_S3_ENDPOINT` | all stages | object-store endpoint URL (MinIO locally; unset ⇒ AWS default) |
| `EFTP_S3_ACCESS_KEY` / `EFTP_S3_SECRET_KEY` | all stages | per-stage scoped credentials |
| `EFTP_S3_REGION` | all stages | region (default `us-east-1`) |
| `MLFLOW_TRACKING_URI` | judge, trainer, smoke | MLflow server URL |
| `EFTP_JUDGE_BASE_URL` | judge | OpenAI-compatible endpoint base URL |
| `EFTP_JUDGE_API_KEY` | judge | key for that endpoint (dummy value ok for local servers) |
| `HF_TOKEN` | tokenizer, trainer, smoke | Hugging Face token for gated models |

### 4.4 CLI

One console script, `eftp`, with subcommands: `ingest`, `clean`, `judge`, `tokenize`, `train`, `smoke`, `run`. Common options on every subcommand: `--run-id` (required except `run`, which generates it), `--config` (default `configs/pipeline.yaml`).
...
### 5.1 StorageClient (`eftp/core/storage.py`)
...
### 5.2 Schemas (`eftp/core/schemas.py`)
...
Single YAML file (default `configs/pipeline.yaml`), validated by a pydantic model in `eftp/core/config.py` (unknown keys are errors). Canonical keys:
...
judge:
  model: ""                     # judge model name at EFTP_JUDGE_BASE_URL (empty ⇒ fail at startup)
...
**MVP scope:** everything in this document except §2's KFP orchestrator and the Inference Engine row of §1.2 is in the MVP. Phase 4 adds `eftp-assets` and multimodal fields, which are already reserved in the schemas.
