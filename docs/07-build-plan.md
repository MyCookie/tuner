...
**Files:** `src/eftp/core/schemas.py`, `src/eftp/core/manifest.py`, `tests/unit/test_schemas.py` (doc-02 examples embedded as fixtures).
...
**Files:** `src/eftp/core/storage.py`, `tests/integration/test_storage.py`.
...
### T06 — Ingestor
**Files:** `src/eftp/ingestor/` (`sources.py`, `cli.py`), `tests/unit/test_sources.py`, `tests/integration/test_ingestor.py`.
**Spec:** [ingestor.md](03-components/ingestor.md) — implement its acceptance criteria as the integration test.
**Verify:** `uv run eftp ingest --run-id $(uv run python -m eftp.core.ids) --config configs/pipeline.yaml` then inspect `eftp-bronze` in the MinIO console.

### T07 — Cleaner
**Files:** `src/eftp/cleaner/` (`patterns.py`, `rules.py`, `cli.py`), `tests/unit/test_cleaner_rules.py`, `tests/integration/test_cleaner.py`.
**Spec:** [cleaner.md](03-components/cleaner.md).
**Verify:** integration test asserts drop counts equal `expected_counts.json`.

### T08 — Judge
**Files:** `src/eftp/judge/` (`client.py`, `prompts.py`, `cli.py`), `tests/unit/test_judge_parsing.py`, `tests/integration/test_judge.py` (against mock judge).
...
### T10 — Tokenizer
**Files:** `src/eftp/tokenizer/` (`masking.py`, `split.py`, `cli.py`), `tests/unit/test_split.py`, `tests/unit/test_masking.py`, `tests/integration/test_tokenizer.py` (uses `tiny-test` adapter).
...
### T11 — Trainer
**Files:** `docker/trainer.Dockerfile`, `src/eftp/trainer/cli.py`, `tests/integration/test_trainer.py` (tiny-test adapter, 1 epoch, CPU-capable).
...
### T12 — Smoke-test
**Files:** `src/eftp/smoke/cli.py`, `tests/integration/test_smoke.py`.
**Spec:** [smoke-test.md](03-components/smoke-test.md).
**Verify:** transcript at `eftp-artifacts/{run_id}/smoke/transcript.json` and attached to the trainer's MLflow run.

### T13 — Driver + registry list
**Files:** `eftp run` implementation in `src/eftp/cli.py`, `src/eftp/registry_ops/cli.py` (`eftp registry list` only, [registry.md MVP scope](03-components/registry.md)).
...
- **Slice 4 (Phase 4):** `eftp-assets` + multimodal ingestion/cleaning/eval; multimodal adapter fields ([04 §5](04-model-adapters.md)); multimodal fine-tune.
