...
from tuner.core.buckets import BRONZE
from tuner.core.config import DEFAULT_CONFIG_PATH, ConfigError, IngestSourceConfig, load_config
from tuner.core.ids import canonical_hash, new_record_id, utc_now, validate_run_id_option
from tuner.core.manifest import records_hash, shard_bytes
...
def _write_shard(
    ...
    storage.write_jsonl(BRONZE, f"{run_id}/{key_name}", records)
    return key_name, shard_bytes(records)
...
def ingest(
    ...
    try:
        storage.delete_prefix(BRONZE, f"{run_id}/")
        ...
        storage.write_json(BRONZE, f"{run_id}/manifest.json", manifest.model_dump(mode="json"))
