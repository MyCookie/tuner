...
def smoke(
    ...
    """Run the Smoke-test end to end; returns the process exit code (0/2/3)."""
    try:
    ...
    with tempfile.TemporaryDirectory() as work_dir_str:
        ...
        # Checked before load_tokenizer/load_base_model below -- "no model download
        # happens" for this case (SMK-I-005), same discrimination fix TRN-I-006
        # needed (PR #11 review round 1 finding 2).
        ...
        for entry in selected:
            ...
            # prompt_messages is exactly what the model is prompted with (02 §5.3).
        ...
        if config.train.method == "full":
            # No PEFT adapter for full FT -- the saved weights *are* the tuned model
            # (02 §5.3's train.method: full clarification).
            tuned_model = AutoModelForCausalLM.from_pretrained(str(weights_dir))
        else:  # pragma: no cover -- GPU-only (PEFT/QLoRA attach); manual T15 check (08 smoke.md)
        ...
        # Exactly one run must match (03-components/smoke-test.md Output; the pair
        # filter, not tuner.run_id alone, per 01 §7 -- checked before anything is
        # written to storage so a bad lookup never leaves a committed transcript
        # dangling without its MLflow attachment.
        if len(runs) != 1:
            click.echo(
                f"smoke: expected exactly one trainer run for run_id {run_id}, found {len(runs)}",
                err=True,
            )
            return 2
