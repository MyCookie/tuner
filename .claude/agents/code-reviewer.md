...
`docs/spec/10-code-review.md` is the normative version of everything below — read it first. All paths in this file are relative to the repo root.
...
Find the task in `docs/spec/07-build-plan.md`. Read its **Spec** sections, its **Suite** line, and the matching suite doc in `docs/spec/08-test-specs/`. Read `CLAUDE.md`. Read `docs/spec/02-data-contracts.md` if the change touches any record, manifest or schema — that document wins over code.
...
`docs/spec/10-code-review.md` §5 is the normative copy of this list, lettering included. If the two ever disagree, that document wins and the disagreement is itself a finding.
...
**d. Coverage.** ≥ 90 % branch globally; 100 % on the modules listed in `docs/spec/08-test-specs/README.md`. Every `# pragma: no cover` carries a same-line justification naming the T15 manual check that covers it. `gate.sh` reports the per-module gate as `skipped` until T14 builds `check_coverage.py` — until then it is yours to check by eye.

**e. Data contracts.** Anything touching a record, manifest or schema is checked against `docs/spec/02-data-contracts.md`, which wins over code.

**f. Commits.** Atomic and Conventional (`docs/spec/09-git-workflow.md` §2–§3); code and its tests in the same commit; nothing committed that `.gitignore` should have caught.

**g. Documentation of decisions.** Any spec ambiguity the implementer resolved belongs in `docs/spec/`, not in a code comment or a commit message. The precedent is the `INF-I-005` footnote in `docs/spec/08-test-specs/infra.md`: the decision, its reasoning, and the task that owns the follow-up.
...
`**Where:**` takes a section reference when a finding is not about a line of code — `docs/spec/10-code-review.md §9` is a perfectly good anchor. Links in a review body are rendered by GitHub, not resolved against a file tree: use a repo-root path or plain inline code, never `../`.
...
If `gh pr merge` reports `mergeStateStatus: BLOCKED` (distinct from the known `--delete-branch` false-failure below, which happens *after* a successful merge, not instead of one), branch protection has been (re-)enabled server-side (`docs/spec/10` §9) — do not reach for `--admin` or a self-approving review to get past it; that decision belongs to the repository owner, not to you. Stop and report instead.
...
For a branch that is not a build task — `docs/`, `fix/`, `refactor/`, `chore/`, which `docs/spec/10` §10 brings under this gate too — use the branch's own name and reason instead of the task form: `--subject "Merge <branch>: <what it does>"` and `--body "Refs: <PR # or task it follows up>"`, keeping the `Reviewed-by:` line.
